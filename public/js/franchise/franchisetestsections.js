...
var total_Section = $('#test_no_of_sections').val();
...
function onTriggerQuestionType(){
    ...
    if ($("#test_no_of_sections").val() == 1)  {
        console.log($("#test_no_of_sections").val());
        total_questions = parseInt(total_questions);
        $('#number_of_questions_0').empty();
        
        for (let i = 1; i <= total_questions; i++) {
            var options = "<option "+(i == total_questions ? 'selected' : '')+" value='"+ i +"' >"+ i +" Questions</option>";
            // 
            $('#number_of_questions_0').append(options);
        }
        $('#number_of_questions_0').val(total_questions);

    }
    else {
        $('#number_of_questions_0').append('<option value=""></option>');
        for (let i = 1; i <= total_questions; i++) {
...
async function onChangeQustions(no_of_questions, key) {
     var q_count = 0
     for (let index = 0; index < total_Section; index++) {
        q_count=q_count+($('#number_of_questions_'+index).val()?parseInt($('#number_of_questions_'+index).val()):0)
    }
    var key = parseInt(key);
    var t_questions = 0;
    key+=1;
    console.log("t_questions",total_questions,q_count);
    var aa = total_questions-q_count
    ...
    if (t_questions == 0) {
        ...
        // for (let index =key; index < total_Section; index++) {
            $('#number_of_questions_'+key).removeAttr('disabled');
            $('#number_of_questions_'+key).empty();
            var option= '<option value=""></option'
            for (let i = 1; i <= t_questions; i++) {
                console.log("total_Section",total_Section-2);
                console.log("key",key-1);
                console.log("key",i);
                if(total_Section-2==key-1 && i==t_questions ){
                option += '<option value="'+ i +'" selected>'+ i +' Questions</option>';
                }else{
