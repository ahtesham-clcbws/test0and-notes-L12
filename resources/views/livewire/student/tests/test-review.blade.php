<div class="min-h-screen bg-[#f8fafc] flex flex-col font-sans" wire:ignore.self>
    ...
    <div class="flex-1 bg-white p-6 max-w-5xl mx-auto w-full flex flex-col justify-start">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center px-4 max-w-4xl mx-auto w-full mt-4">
            <!-- Completion Card -->
            <div class="bg-[#d8f0fa] border border-[#b2e0f4] rounded-2xl p-6 flex items-center justify-between shadow-sm min-h-[140px] w-full">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 md:col-span-2 w-full">
                <!-- Attempted -->
                <div class="bg-[#f0fdf4] border-[2px] border-[#86efac] px-5 py-4 rounded-xl flex justify-between items-center shadow-sm">
                ...
                <!-- Not Attempted -->
                <div class="bg-[#f9fafb] border-[2px] border-[#d1d5db] px-5 py-4 rounded-xl flex justify-between items-center shadow-sm">
                ...
                <!-- Total Visit -->
                <div class="bg-[#f5f3ff] border-[2px] border-[#ddd6fe] px-5 py-4 rounded-xl flex justify-between items-center shadow-sm">
            ...
            <div class="flex flex-wrap justify-center gap-3">
                ...
                @foreach ($allQuestions as $index => $question)
                    @php
                        ...
                        if ($isAnswered) {
                            $circleBg = 'bg-[#16a34a] text-white ' . ($isMarked ? 'border-[3px] border-yellow-400 shadow-sm hover:scale-110 hover:bg-[#15803d] cursor-pointer' : 'border-[#16a34a] opacity-60 cursor-not-allowed');
                        } elseif ($isVisited) {
                            $circleBg = 'bg-gray-400 text-white ' . ($isMarked ? 'border-[3px] border-yellow-400 shadow-sm hover:scale-110 hover:bg-gray-500 cursor-pointer' : 'border-gray-400 opacity-60 cursor-not-allowed');
                        } else {
                            $circleBg = 'bg-white text-gray-400 ' . ($isMarked ? 'border-[3px] border-yellow-400 shadow-sm hover:scale-110 hover:bg-gray-50 cursor-pointer' : 'border-gray-300 opacity-60 cursor-not-allowed');
                        }
                    @endphp
                    <button 
                        @if($isMarked) wire:click="viewSolution({{ $question->id }})" @else disabled @endif
                        class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs shadow-sm transition-all {{ $circleBg }}"
                        title="Question {{ $index + 1 }} {{ !$isMarked ? '(Locked)' : '' }}"
                    >
                        {{ $index + 1 }}
                    </button>
                @endforeach
        ...
        <div class="max-w-4xl mx-auto w-full mt-8 px-4">
            <div class="bg-blue-50 border-l-4 border-blue-500 rounded-2xl p-5 flex gap-4 items-start shadow-sm">
                <div class="bg-blue-500 text-white p-2 rounded-xl flex-shrink-0">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-blue-800 text-sm font-medium mt-1 leading-relaxed">
                        Click on any question bubble above to review your recorded choice. 
                        For questions <strong class="text-yellow-600 font-black">Marked for Review</strong> (yellow bordered), you can change and submit your answer directly here in the popup.
                    </p>
                </div>
            </div>
        </div>
