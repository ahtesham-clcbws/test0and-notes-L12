        ...
        <div class="dashboard-container mb-5">
            ...
            @if(isset($submit_content) && $submit_content == 1)
                ...
                <div class="" style="text-align: right;margin-top: 10px;margin-right: 28px;">
                    <a href="{{ route('franchise.management.creater.material_add') }}" class="btn btn-success pull-right">Add Study Material</a>
                </div>
                @endif
                <div class="card-body">
...
<script>
    $(document).ready(function () {
        var table = $('#studytable').DataTable({
            ...
            bprocessing: true,
            serverSide: true,
            ajax: '{!! route("franchise.management.creater.material") !!}',
     ...
     $(document).on('click', '.delete_material', function () {
        ...
        if (id != "") {
            ...
            $.ajax({
                type: "POST",
                url: '{!! route("franchise.management.creater.material_delete") !!}',
                dataType: "JSON",
                data: { study_material_id: id, file: file},
                success: function (data) {
                    if(data['status'] == 200){
                        alert(data['message']);
                        location.href = '{!! route("franchise.management.creater.material") !!}';
                    }else{
                        alert(data['message']);
                        location.href = '{!! route("franchise.management.creater.material") !!}';
                    }
