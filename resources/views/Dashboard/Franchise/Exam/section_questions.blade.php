                        ...
                        <div class="card-body">
                            ...
                            <div class="btn-group mb-2 me-3" role="group">
                                ...
                                @if ($data['section']->number_of_questions == $data['total_questions'])
                                    <button type="button" class="btn btn-danger">
                                        Duration {{ $data['section']->number_of_questions * $data['section']->duration }} Minutes
                                    </button>
                                @endif
                                <button type="button" class="btn btn-danger">
                                    {{ $data['section']->duration }} Minutes/ Question
                                </button>
                            </div>
                                ...
                                <table class="table" id="questionsTable">
                                    ...
                                    <tbody>
                                        @foreach ($data['questions'] as $key => $question)
                                            <tr>
                                                <td scope="col" class="table-img">{{ $question_count+=1 }}</td>
                                                <td scope="col" class="table-img">{!! $question->question->question !!}</td>
                                                <td scope="col" class="table-img">{!! $question->question->mcq_answer !!}</td>
                                                <td scope="col" class="table-img">{!! $question->question->solution !!}</td>
                                                <!-- <td scope="col">
                                                ...
                                                <td class="text-end">
                                                    ...
                                                    {{-- @if ($question->question->status == 'pending' && $question->question->creator_id == $data['auth_id'])
                                                        <button class="btn btn-link btn-sm"><i
                                                                class="bi bi-pencil-square text-primary"></i></button>
                                                    @endif --}}
                            ...
                            <table class="table" id="questionsBankTable">
                                ...
                                <tbody>
                                    @foreach ($data['unused_questions'] as $key => $question)
                                        <tr>
                                            <td scope="col" class="table-img">{{ $key+=1 }}</td>
                                            <td scope="col" class="table-img">{!! $question->classGroup->name !!}</td>
                                            <td scope="col" class="table-img">{!! $question->question !!}</td>
                                            <td scope="col" class="table-img">{!! $question->mcq_answer !!}</td>
                                            <td scope="col" class="table-img">{!! $question->solution !!}</td>
                                            <td class="text-end">
                                                @if ($data['section']->number_of_questions > $data['total_questions'])
                ...
                <form class="modal-content" id="questionForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-none" id="questionHiddenFields">
                            ...
                            <input name="education_type_id" value="{{ $data['test']->Educationtype->id }}"
                                class="d-none">
                            <input name="class_group_exam_id" value="{{ $data['test']->EducationClass->id }}"
                                class="d-none">
                            <input name="board_agency_state_id" value="{{ $data['test']->EducationBoard->id }}"
                                class="d-none">
                            <input name="subject" value="{{ $data['section']->sectionSubject->id }}"
                                class="d-none">
                            @if ($data['section']->subject_part)
                                <input name="subject_part" value="{{ $data['section']->sectionSubjectPart->id }}"
                                    class="d-none">
                            @endif
                            @if ($data['section']->subject_part_lesson)
                                <input name="subject_lesson_chapter"
                                    value="{{ $data['section']->sectionSubjectLesson->id }}" class="d-none">
                            @endif
                            <input name="question_type" value="{{ $data['section']->question_type }}"
                                class="d-none">
                            @if ($data['section']->question_type == '1')
                                <input name="mcq_options" value="{{ $data['section']->mcq_options }}"
                                    class="d-none">
                            @endif
                        </div>
                    </div>
...
@section('javascript')
    ...
    <script type="text/javascript" src="{{ asset('js/admintestsectionquestions.js') }}"></script>
    <script>
        $("#submit_question").click(function(){
            window.location = "{{ route('franchise.dashboard_tests_list') }}";
        });
    </script>
@endsection
