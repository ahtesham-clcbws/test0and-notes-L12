...
<body>
    ...
    <script>
        ...
        var subject_part = <?php if(isset($subject_part)) echo $subject_part; else echo 0; ?>;
         var video = "<?php if(isset($video)) echo $video; else echo 0; ?>";
        var notes = "<?php if(isset($notes)) echo $notes; else echo 0; ?>";
        var gk = "<?php if(isset($gk)) echo $gk; else echo 0; ?>";
        var test = <?php if(isset($test)) echo $test; else echo 0; ?>;
        var package = "<?php if(isset($package)) echo $package; else echo 0; ?>";
        ...
        if ($('.select2').length) {console.log("select2");
            $('.select2').select2();
        }
        if ($('.datatable').length) {
            $('.datatable').DataTable();
...
async function getStudyMaterial(id) {
    ...
    await $.ajax({
        ...
        contentType: false,
        processData: false
    }).done(function (data) { console.log("study material data:", data);
            ...
            if (study_material.length > 0) {
                ...
                $(study_material).each(function (index, item) {
                    if(item.category == 'Live & Video Classes'){
                        ...
                        videoOptions += '<option value="' + item.id + '" '+selected+'>' + item.title + '</option>';
                    }
                    if(item.category == 'Static GK & Current Affairs'){
                        ...
                        staticOptions += '<option value="' + item.id + '" '+selected+'>' + item.title + '</option>';
                    }
                    if(item.category == 'Study Notes & E-Books'){
                        ...
                        notesOptions += '<option value="' + item.id + '" '+selected+'>' + item.title + '</option>';
                    }
                });
            } else {
...
async function getTest(id) {
    ...
    await $.ajax({
        ...
        contentType: false,
        processData: false
    }).done(function (data) { console.log("Get Test data:", data);
        if (data && data.success) {
            ...
            if (test_data.length > 0) {
                ...
                $(test_data).each(function (index, item) {
                        ...
                        testOptions += '<option value="' + item.id + '" '+selected+'>' + title + '</option>';
                });
            } else {
                alert('No Data, please select another, or add some.');
            }
...
if(test_type != '' && class_id != ''){
    ...
    await $.ajax({
        ...
        contentType: false,
        processData: false
    }).done(function (data) { console.log("Get package data:", data);
        if (data && data.success) {
            ...
            if (package_data.length > 0) {
                ...
                $(package_data).each(function (index, item) {
                        ...
                        packageOptions += '<option value="' + item.id + '" '+selected+'>' + item.plan_name + '</option>';
                });
            } else {
                alert('No Data, please select another, or add some.');
            }
