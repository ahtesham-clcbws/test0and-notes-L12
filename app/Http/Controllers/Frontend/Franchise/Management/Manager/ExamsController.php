...
namespace App\Http\Controllers\Frontend\Franchise\Management\Manager;
...
class ExamsController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        if ($req->isMethod('post')) {
            ...
            if (!empty($search_value)) {
                $testTableData = TestModal::select(['test.id as id', 'test_section.id as section_id','test.title as title', 'test.sections as sections', 'test.total_questions as total_questions',
                'test.questions_submitted as questions_submitted', 'test.questions_approved as questions_approved', 'test.reviewed as reviewed', 'test.reviewed_status as reviewed_status',
                'test.published as published','test.created_at as created_at','test.education_type_child_id as education_type_child_id','test.published_status as published_status',
                'users.name as username','test_section.number_of_questions as section_questions','test_section.sent_to_publisher as sent_to_publisher'])
                ...
                'test.questions_submitted as questions_submitted', 'test.questions_approved as questions_approved', 'test.reviewed as reviewed', 'test.reviewed_status as reviewed_status',
                'test.published as published','test.created_at as created_at','test.education_type_child_id as education_type_child_id','test.published_status as published_status',
                'users.name as username','test_section.number_of_questions as section_questions','test_section.sent_to_publisher as sent_to_publisher','test_section.creator_id as creator_id','test_section.publisher_id as publisher_id'])
                ->leftJoin('test_section','test_section.test_id','test.id')
                ->leftJoin('users','users.id','test.user_id')
            ...
            foreach ($testTableData as $key => $testData) {
                ...
                // }
                // dd();
                $total_questions = $testData['section_questions'];
                // dd($testData['section_questions']);
                $testTableData[$key]['total_questions'] = $testData['section_questions'] . ' / ' . $testData->getSection->find($testData['section_id'])->getQuestions()->wherePivot('deleted_at','=',NULL)->count();
                $status = '';

                // dd($testData->getQuestions()->wherePivot('deleted_at','=',NULL)->count());
                // == 'true' ? $testData->getQuestions()->wherePivot('deleted_at','=',NULL)->count()  : '0'
                $publish_count = count($testData->getSection()->select('is_published')->where('is_published',1)->get());
                $creater_count = count($testData->getSection()->select('sent_to_publisher')->where('sent_to_publisher',1)->get());
                ...
                if ($total_questions == 0) {
                    ...
                    if ($total_questions != $testData->getSection->find($testData['section_id'])->getQuestions()->wherePivot('deleted_at','=',NULL)->count() || $total_questions < $testData->getSection->find($testData['section_id'])->getQuestions()->wherePivot('deleted_at','=',NULL)->count()) {
                        ...
                        if($testData['sent_to_publisher'] == 1 || $testData['creator_id'] == Auth::user()->id){
                            $status = '<a><span class="badge bg-primary">Waiting for Approval</span></a>';
                        }

                        if ($testData['sent_to_publisher'] != 1 && $testData['creator_id'] == Auth::user()->id) {
                            $status = '<a href="javascript:void(0);" onclick="sentToPublisher('.$testData['section_id'].')"><span class="badge bg-primary">Sent to Publisher</span></a>';
                        }

                        if($testData['sent_to_publisher'] == 1 && $testData['publisher_id'] == Auth::user()->id) {
                            $status = '<a href="javascript:void(0);" onclick="publishTest('.$testData['section_id'].')"><span class="badge bg-primary">Publish Test</span></a>';
                        }


                        if ($creater_count != $testData['sections'] && $publish_count != $testData['sections']) {
                            $status = '<span class="badge bg-warning text-dark">Created</span>';
                        }

                        if ($creater_count == $testData['sections'] && $publish_count != $testData['sections']) {
                            $status = '<span class="badge bg-warning text-dark">Awaiting Publisher</span>';
                        }

                        if ($creater_count == $testData['sections'] && $publish_count == $testData['sections']) {
                            $status = '<a href="'.route('franchise.management.manager.dashboard_publish_test_exam', [$testData['id']]).'"><span class="badge bg-primary">Publish Test</span></a>';
                        }
                        ...
                        if ($testData['published'] == 1) {
                            $status = '<a href="'.route('franchise.management.manager.dashboard_publish_test_exam', [$testData['id']]).'"><span class="badge bg-success">Published</span></a>';
                        }
                ...
                if ($testData['sections']) {
                    ...
                    // foreach ($sectionsX as $keyX => $sectionX) {
                        $sectionUrl = route('franchise.management.manager.dashboard_test_section_question', [$testData['id'], $testData['section_id']]);
                ...
                $actionsHtml = '<a href="' . route('franchise.management.manager.dashboard_update_test_exam', [$testData['id']]) . '" title="Edit Test"><i class="bi bi-pencil-square text-success me-2"></i></a>
        ...
        // dd($this->data);
        return view('Dashboard/Franchise/Management/Manager/Exam/teststable')->with('data', $this->data);
    }
    ...
    public function saveTest(Request $req, $test_id = 0)
    {
        ...
        if ($req->isMethod('post')) {
            ...
            if ($inputs['form_name'] == 'test_form') {
                ...
                if ($query) {
                    $testId = $testMd->id;
                    return redirect()->route('franchise.management.manager.dashboard_update_test_exam', [$testId])->withErrors(['testSuccess' => 'Test succesfully added.']);
            ...
            if ($inputs['form_name'] == 'sections_form') {
                ...
                return redirect()->route('franchise.management.manager.dashboard_tests_list')->withErrors(['sectionsError' => $returnResponse]);
        ...
        return view('Dashboard/Franchise/Management/Manager/Exam/savetest')->with('data', $this->data);
    }
    ...
    public function publishTest(Request $req,$test_id)
    {
        ...
        if ($req->isMethod('post')) {
            ...
            if ($inputs['form_name'] == 'publish_test') {
                ...
                return redirect()->route('franchise.management.manager.dashboard_tests_list')->withErrors(['testSuccess' => 'Test succesfully added.']);
        ...
        return view('Dashboard/Franchise/Management/Manager/Exam/publish_test')->with('data', $this->data);
    ...
    public function testSections(Request $req, $test_id)
    {
        ...
        if ($req->isMethod('post')) {
            ...
            $alertMessage   = $errorMessage . $successMessage;
            $returnResponse = ['class' => $alertClass, 'message' => $alertMessage];
            return redirect()->route('franchise.management.manager.dashboard_test_sections', [$test_id])->withErrors(['sectionsError' => $returnResponse]);
            return print_r($sections);
        }
        $this->data['test']     = $test;
        $this->data['subjects'] = Subject::get();
        ...
        $this->data['creators'] = $creators;
        $this->data['pagename'] = 'Test Sections';
        return view('Dashboard/Franchise/Management/Manager/Exam/sections')->with('data', $this->data);
    }
    ...
    public function section_questions(Request $req, $test_id, $section_id)
    {
        ...
        return view('Dashboard/Franchise/Management/Manager/Exam/section_questions')->with('data', $this->data);
    }
