    ...
    public function index(Request $request)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($request->input('form_name') == 'admin_login') {
                $userData = ['email' => $request->input('email'), 'password' => $request->input('password'), 'isAdminAllowed' => 1];
                ...
                if ($package_type == 0) {
                    $testData = TestModal::select(['test.id', 'test.title', 'franchise_details.institute_name as institute_name'])
                        ->leftJoin('franchise_details', function ($join) {
                            $join->on('franchise_details.user_id', 'test.user_id')
                    ...
                    $testData = TestModal::join('franchise_details', function ($join) {
                        $join->on('franchise_details.user_id', 'test.user_id')
                            ->whereNull('franchise_details.deleted_at');
                    })
                        ->where('education_type_id', '=', $educationId)
